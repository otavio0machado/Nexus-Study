...
export const Flashcards: React.FC = () => {
  ...
  const progressPercent = Math.min(
    100,
    Math.round(
      ((dailyProgress.newStudied + dailyProgress.reviewStudied) /
        (state.settings.maxNewPerDay + state.settings.maxReviewsPerDay)) *
        100
    )
  );
    ...
    <div className="space-y-6 animate-fade-in h-full flex flex-col pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
        <div className="glass-panel p-4 rounded-xl flex items-center gap-4 relative overflow-hidden group">
          <div
            className="p-3 bg-orange-500/10 rounded-full text-orange-500 group-hover:scale-110 transition-transform effect-levitate aura-orange"
        ...
        <div className="glass-panel p-4 rounded-xl flex items-center gap-4 group">
          <div
            className="p-3 bg-yellow-500/10 rounded-full text-yellow-500 group-hover:scale-110 transition-transform effect-levitate aura-yellow"
        ...
        <div className="glass-panel p-4 rounded-xl flex items-center gap-4 group">
          <div
            className="p-3 bg-cyan-500/10 rounded-full text-cyan-500 group-hover:scale-110 transition-transform effect-levitate aura-cyan"
      ...
      <div className="flex items-center justify-between">
        ...
        {!activeDeck ? (
          <div className="flex gap-2">
            ...
            >
              <Plus size={18} />{" "}
              <span className="hidden md:inline">Baralho</span>
            </button>
          </div>
        ) : (
          <div className="flex gap-2">
          ...
          {activeDeck.cards.length === 0 ? (
            ...
            activeDeck.cards.map((card) => {
              ...
              return (
                <div
                  key={card.id}
                  className="glass-panel p-4 rounded-lg flex flex-col md:flex-row gap-4 border-l-4 border-white/5 hover:border-nexus-500/50 transition-colors"
                >
                  ...
                  {card.cardType === "basic" && (
                    <>
                      ...
                      <div className="flex-1">
                        <span className="text-[10px] text-slate-500 uppercase tracking-wider mb-1 block font-bold">
                          Verso
                        </span>
                        <p className="font-medium text-slate-300 whitespace-pre-wrap">
                          {card.back}
                        </p>
                      </div>
                    </>
          ...
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-lg p-6 shadow-2xl">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-nexus-600 hover:bg-nexus-500 text-white rounded-lg"
              >
                Adicionar
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-md p-6 shadow-2xl">
            ...
            <input
              type="text"
              placeholder="Título"
              className="w-full bg-slate-800 border border-slate-700 rounded-lg p-3 mb-3 text-white focus:ring-2 focus:ring-nexus-500 outline-none"
              value={newDeckTitle}
